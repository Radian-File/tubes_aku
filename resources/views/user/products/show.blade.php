...
@section('content')
    <div class="dashboard-header d-flex justify-content-between align-items-center flex-wrap">
        ...
        <a href="{{ route('user.dashboard') }}" class="btn btn-back"><i class="fas fa-arrow-left me-2"></i>Kembali ke
            Produk</a>
    </div>
    @if (session('success'))
    ...
    <div class="product-detail-card">
        <div class="row">
            <div class="col-md-6">
                @if ($product->gambar)
                    <img src="{{ asset('storage/' . $product->gambar) }}" class="product-image w-100"
                        alt="{{ $product->nama }}">
                @else
            ...
            <div class="col-md-6">
                ...
                <!-- <p class="product-info"><strong>Status:</strong> {{ $product->status_pesanan }}</p> -->
                ...
                <div class="mt-4">
                    <a href="{{ route('user.products.buy', $product->id) }}" class="btn btn-add"><i
                            class="fas fa-shopping-cart me-2"></i>Beli</a>
                </div>
            </div>
        </div>
    </div>
